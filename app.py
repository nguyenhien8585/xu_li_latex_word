...
def detect_markdown_table(text):
    """Phát hiện bảng Markdown trong văn bản"""
    try:
        lines = text.split('\n')
        table_lines = []
        ...
        for i, line in enumerate(lines):
            line = line.strip()
            # Kiểm tra có phải dòng bảng không (có ít nhất 2 dấu |)
            if line.count('|') >= 2 and line.startswith('|') and line.endswith('|'):
                # Kiểm tra không phải dòng separator (:---, :---:, ---)
                content = line[1:-1]  # Bỏ | đầu và cuối
                cells = [cell.strip() for cell in content.split('|')]
                
                # Nếu không phải dòng separator
                is_separator = True
                for cell in cells:
                ...
                if not is_separator:
                    table_lines.append((i, line, cells))
        
        return table_lines if len(table_lines) >= 2 else []  # Ít nhất 2 dòng mới là bảng
        
    except Exception as e:
        return []

def parse_markdown_table(table_lines):
    """Phân tích cấu trúc bảng Markdown"""
    try:
        if not table_lines:
            return None
            
        # Lấy số cột từ dòng đầu tiên
        max_cols = len(table_lines[0][2])
        
        # Chuẩn hóa tất cả các dòng có cùng số cột
        table_data = []
        for line_no, line_text, cells in table_lines:
            # Đảm bảo đủ số cột
...
def create_word_table_from_markdown(new_doc, table_info, options):
    """Tạo bảng Word từ thông tin bảng Markdown"""
    try:
        if not table_info:
            return None
            
        # Tạo bảng mới
        new_table = new_doc.add_table(rows=table_info['rows'], cols=table_info['cols'])
        
        # Điền dữ liệu vào bảng
        for i, row_data in enumerate(table_info['data']):
            for j, cell_data in enumerate(row_data):
                if i < len(new_table.rows) and j < len(new_table.rows[i].cells):
                    ...
                    cell.text = ""  # Xóa nội dung cũ
                    paragraph = cell.paragraphs[0]
                    
                    # Tìm và xử lý LaTeX trong cell
                    latex_expressions = find_latex_expressions(cell_text)
                    
                    if latex_expressions:
                        last_idx = 0
                        
                        for start, end, latex_expr, math_type in latex_expressions:
                            # Thêm văn bản trước LaTeX
                            if start > last_idx:
                                text_before = cell_text[last_idx:start]
                                if text_before:
                            ...
                            if omml_xml:
                                insert_equation_in_paragraph(paragraph, omml_xml)
                            else:
                        ...
                        if last_idx < len(cell_text):
                            remaining_text = cell_text[last_idx:]
                            if remaining_text:
                                paragraph.add_run(remaining_text)
                    else:
                        # Không có LaTeX, thêm văn bản thường
...
def process_markdown_tables_in_text(text, new_doc, options):
    """Xử lý bảng Markdown trong văn bản và trả về văn bản đã xử lý + số bảng"""
    try:
        lines = text.split('\n')
        table_lines = detect_markdown_table(text)
        
        if not table_lines:
            return text, 0
        
        # Nhóm các dòng liên tiếp thành bảng
        tables = []
        current_table = []
        
        for line_no, line_text, cells in table_lines:
            if not current_table or line_no == current_table[-1][0] + 1:
                current_table.append((line_no, line_text, cells))
            else:
                # Kết thúc bảng hiện tại, bắt đầu bảng mới
                if current_table:
                    tables.append(current_table)
                current_table = [(line_no, line_text, cells)]
        ...
        for table_lines_group in reversed(tables):
            table_info = parse_markdown_table(table_lines_group)
            if table_info:
                # Tạo bảng Word
                word_table = create_word_table_from_markdown(new_doc, table_info, options)
                if word_table:
                    tables_created += 1
                    
                    # Xóa các dòng bảng Markdown khỏi text
                    ...
                    for i in range(end_line, start_line - 1, -1):
                        if i < len(processed_lines):
                            if i == start_line:
                                processed_lines[i] = f"[TABLE_PLACEHOLDER_{tables_created}]"
                            else:
                                processed_lines.pop(i)
        
        # Ghép lại text
        processed_text = '\n'.join(processed_lines)
        return processed_text, tables_created
...
def process_word_document_safe(doc, options):
    ...
    try:
        ...
        # Thu thập tất cả text để xử lý markdown tables
        all_text = ""
        paragraph_texts = []
        ...
        if options.get('convert_markdown_tables', True):
            processed_text, markdown_tables = process_markdown_tables_in_text(all_text, new_doc, options)
            # Cập nhật lại paragraph_texts sau khi xử lý markdown tables
            if markdown_tables > 0:
                updated_lines = processed_text.split('\n')
                paragraph_texts = updated_lines[:len(paragraph_texts)]
        
        # Xử lý từng paragraph
        for i, para in enumerate(doc.paragraphs):
            try:
                # Lấy text đã được xử lý markdown tables
                if i < len(paragraph_texts):
                    text = paragraph_texts[i]
                else:
                    text = para.text
                
                # Bỏ qua các placeholder table
                if text.startswith('[TABLE_PLACEHOLDER_'):
                    continue
        ...
        # Xử lý bảng gốc (không phải markdown tables)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("**📊 Xử lý Bảng:**")
        convert_markdown_tables = st.checkbox("Chuyển đổi Markdown Tables", value=True,
                                            help="Chuyển |...|...| thành bảng Word thực sự")
...
def main():
 ...
 → a+b (inline mới)
        - `$\\frac{a}{b}$` → phân số (display)
        
        **Markdown Tables:**
        - `|Col1|Col2|` → Bảng Word thực
        - Tự động format header + LaTeX trong cell
        """)
        
        st.markdown("---")
        st.markdown("### 🆕 Tính năng mới")
        st.info("""
        ✅ **LaTeX ${...}$**: Định dạng mới cho công thức
        ✅ **Markdown Tables**: Chuyển |...|...| thành bảng Word
        ✅ **LaTeX trong bảng**: Tự động xử lý công thức trong cell
        """)
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Xử lý file", type="primary", use_container_width=True):
                options = {
                    ...
                    'convert_markdown_tables': convert_markdown_tables
                }
                ...
                try:
                    with st.spinner("⏳ Đang xử lý file, vui lòng chờ..."):
                        ...
                        with metric_cols[1]:
                            ...
                            st.metric("📊 Bảng Markdown", stats['markdown_tables'])
                        with metric_cols[2]:
                            st.metric("🏗️ Bảng Word gốc", stats['formatted_tables'])
                        ...
                        with metric_cols[3]:
                            ...
                            total_tables = stats['markdown_tables'] + stats['formatted_tables']
                            st.metric("📊 Tổng bảng", total_tables)
                        ...
                        if stats['markdown_tables'] > 0:
                            success_messages.append(f"📊 {stats['markdown_tables']} bảng Markdown")
                        if stats['formatted_tables'] > 0:
                            success_messages.append(f"🏗️ {stats['formatted_tables']} bảng Word")
    ...
    with col2:
        ...
        with st.expander("🔢 Định dạng LaTeX mới"):
            st.markdown("""
            **Các định dạng được hỗ trợ:**
            ```
            $x^2 + y^2 = z^2$        → Inline cũ
            ${a + b = c}$            → Inline mới  
            $\\frac{a}{b}$         → Display
            ```
            
            **Ưu điểm ${...}$:**
            - Rõ ràng hơn khi có nhiều ký tự
            - Tránh nhầm lẫn với ký hiệu khác
            - Dễ phân biệt với văn bản thường
            """)
        
        with st.expander("📊 Markdown Tables → Word"):
            st.markdown("""
            **Input (Markdown):**
            ```
            | Lương tháng | ${[6;8)}$ | ${[8;10)}$ |
            | :---: | :---: | :---: |
            | Số nhân viên | 3 | 6 |
            ```
            
            **Output (Word):**
            - Bảng Word thực sự (có thể chỉnh sửa)
            - Header in đậm + tô màu xanh
            - LaTeX trong cell tự động chuyển đổi
            - Căn chỉnh chuẩn, font Times New Roman
            """)
        ...
        with st.expander("⚡ Ví dụ tổng hợp"):
            st.code("""
# LaTeX cơ bản
$x + y = z$
${a^2 + b^2 = c^2}$
...
D. 12
        ...
        with st.expander("🎯 Kết hợp tất cả tính năng"):
            st.markdown("""
            **File Word hoàn chỉnh có thể chứa:**
            
            🔢 **LaTeX đa dạng**: `$...import streamlit as st
from docx import Document
from docx.shared import Inches, Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import nsdecls, qn
...
, `$...$`
            ...
            📊 **Bảng Markdown**: Chuyển text thành bảng thực
            
            📋 **Câu hỏi chuẩn**: Format tự động
            
            🎨 **Định dạng đẹp**: Font, màu sắc, căn chỉnh
            
            → **Kết quả**: File Word professional, sẵn sàng in hoặc chia sẻ!
            """)
    ...
    <div style="text-align: center; color: #666; padding: 20px;">
        <p>🚀 Được phát triển với Streamlit | 📊 Hỗ trợ LaTeX, Markdown Tables & Trắc nghiệm</p>
        <p><small>Phiên bản 4.0 - Mới: LaTeX ${...}$ và Markdown Tables |...|</small></p>
    </div>
    """, unsafe_allow_html=True)
...
if __name__ == "__main__":
    main(), start_pos + 2)
            ...
            latex_expr = text[start_pos + 2:end_pos]
            # Kiểm tra không trùng lặp với $...$
            is_overlap = False
            for existing_start, existing_end, _, _ in expressions:
                if start_pos >= existing_start and end_pos + 2 <= existing_end:
                    is_overlap = True
                    break
