    ...
    try:
        ...
        if len(table.rows) > 0:
            ...
            for cell in header_row.cells:
                # Xóa nội dung cũ và thêm lại với format mới
                cell_text = cell.text
                cell.text = ""
...
def format_questions(paragraph, options):
    ...
    question_pattern = r'^(\*\*Câu\s+\d+:\*\*|Câu\s+\d+:)\s*(.*)
...
def fix_formatting(doc, options):
    ...
    for paragraph in doc.paragraphs:
        ...
        # Sửa font chữ cho các run (chỉ nếu chưa được format)
        has_formatted_content = False
        for run in paragraph.runs:
            if run.font.color.rgb == RGBColor(0, 123, 191):  # Đã format màu xanh
...
def copy_table_to_new_doc(original_table, new_doc, options):
    """Sao chép bảng từ tài liệu gốc sang tài liệu mới với format"""
    try:
        # Lấy kích thước bảng
        rows_count = len(original_table.rows)
        cols_count = len(original_table.columns) if original_table.rows else 0
        ...
        for i, row in enumerate(original_table.rows):
            for j, cell in enumerate(row.cells):
                if i < len(new_table.rows) and j < len(new_table.rows[i].cells):
                    ...
                    new_table.cell(i, j).text = cell_text.strip()
        
        # Format bảng nếu được yêu cầu
        if options.get('format_tables', False):
            format_table(new_table, options)
        
        return new_table
        
    except Exception as e:
        st.error(f"Lỗi sao chép bảng: {str(e)}")
        return None
...
def process_word_document(doc, options):
    ...
    # Lấy tất cả elements trong tài liệu (paragraphs và tables)
    doc_elements = []
    
    try:
        # Thêm paragraphs và tables theo thứ tự xuất hiện
        ...
        # Nếu có lỗi khi lấy elements, xử lý theo cách cũ
        for para in doc.paragraphs:
            doc_elements.append(('paragraph', para))
        for table in doc.tables:
            doc_elements.append(('table', table))
    
    # Xử lý từng element theo thứ tự
    for element_type, element in doc_elements:
        if element_type == 'paragraph':
            ...
            if options.get('format_questions', False):
                ...
                if format_questions_simple(new_para, options):
...
def format_questions(paragraph, options):
    ...
    # Pattern cho câu hỏi (Câu 1:, Câu 2:, etc.) - escape đúng cách
    question_pattern = r'^(\*\*Câu\s+\d+:\*\*|Câu\s+\d+:)\s*(.*)
...
if __name__ == "__main__":
    main(), 'display'),  # Toán học hiển thị
                (r'\$(.+?)\
...
if __name__ == "__main__":
    ...
    question_match = re.match(question_pattern, text, re.IGNORECASE | re.DOTALL)
    if question_match:
        paragraph.clear()
        
        question_part = question_match.group(1)
        content_part = question_match.group(2).strip()
    ...
    answer_match = re.match(answer_pattern, text)
    if answer_match:
        paragraph.clear()
        
        answer_letter = answer_match.group(1)  # A., B., C., D.
        answer_content = answer_match.group(2).strip()  # Nội dung đáp án
...
if __name__ == "__main__":
    main(), 'inline')        # Toán học nội tuyến
            ]
            ...
            # Nếu không tìm thấy biểu thức LaTeX và không format câu hỏi, sao chép đoạn văn như cũ
            if not matches and not options.get('format_questions', False):
                new_para.clear()
                for run in para.runs:
                    new_run = new_para.add_run(run.text)
                    # Sao chép định dạng run
            ...
            try:
                table = element
                new_table = copy_table_to_new_doc(table, new_doc, options)
                if new_table:
                    formatted_tables += 1
            except Exception as e:
                # Nếu có lỗi khi xử lý bảng, bỏ qua
                st.warning(f"Không thể xử lý một bảng: {str(e)}")
                continue
    ...
    return new_doc, {
        ...
        'formatted_questions': formatted_questions,
        'formatted_tables': formatted_tables,
        'total_paragraphs': len([e for e in doc_elements if e[0] == 'paragraph']),
        'total_tables': len([e for e in doc_elements if e[0] == 'table'])
    }
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Nếu có lỗi regex, bỏ qua việc format
        return False
