...
def create_equation_run(paragraph, latex_expr):
    ...
    try:
        ...
        clean_expr = clean_expr.replace('\\alpha', 'α')
        clean_expr = clean_expr.replace('\\beta', 'β')
        clean_expr = clean_expr.replace('\\gamma', 'γ')
        clean_expr = clean_expr.replace('\\delta', 'δ')
        clean_expr = clean_expr.replace('\\theta', 'θ')
        clean_expr = clean_expr.replace('\\lambda', 'λ')
        clean_expr = clean_expr.replace('\\mu', 'μ')
        clean_expr = clean_expr.replace('\\sigma', 'σ')
        clean_expr = clean_expr.replace('\\phi', 'φ')
        clean_expr = clean_expr.replace('\\omega', 'ω')
        
        # Set theory symbols
        clean_expr = clean_expr.replace('\\in', '∈')
        clean_expr = clean_expr.replace('\\notin', '∉')
        clean_expr = clean_expr.replace('\\subset', '⊂')
        clean_expr = clean_expr.replace('\\supset', '⊃')
        clean_expr = clean_expr.replace('\\cup', '∪')
        clean_expr = clean_expr.replace('\\cap', '∩')
        clean_expr = clean_expr.replace('\\emptyset', '∅')
        ...
        clean_expr = clean_expr.replace('\\mathbb{C}', 'ℂ')
        
        # Comparison symbols
        clean_expr = clean_expr.replace('\\leq', '≤')
        clean_expr = clean_expr.replace('\\geq', '≥')
        clean_expr = clean_expr.replace('\\neq', '≠')
        clean_expr = clean_expr.replace('\\approx', '≈')
        clean_expr = clean_expr.replace('\\equiv', '≡')
        
        # Infinity
        clean_expr = clean_expr.replace('\\infty', '∞')
        
        # Plus/minus
        clean_expr = clean_expr.replace('\\pm', '±')
        clean_expr = clean_expr.replace('\\mp', '∓')
        
        # Loại bỏ các ký tự LaTeX còn lại (nhưng giữ lại dấu ngoặc)
        clean_expr = re.sub(r'\\[a-zA-Z]+', '', clean_expr)  # Remove \command
        clean_expr = clean_expr.replace('\\', '')
...
def format_table_properly(table, options):
    """Format bảng đúng cách - chỉ format structure, không xử lý LaTeX (đã được xử lý trước đó)"""
    try:
        ...
        for cell in header_row.cells:
            # Bold and center header - chỉ format, không process content
        ...
        # Format data rows - chỉ format, không process content
        for i in range(1, len(table.rows)):
            row = table.rows[i]
            for cell in row.cells:
                # Center align paragraphs and format fonts
                for paragraph in cell.paragraphs:
                    for run in paragraph.runs:
                        if not run.font.name:
                            run.font.name = 'Times New Roman'
        ...
        return True, 0  # Return 0 for latex count since we don't process LaTeX here anymore
...
def find_latex_in_text(text):
    ...
    # Pattern tìm $...$ và ${...}$ - mở rộng để hỗ trợ nhiều ký tự hơn
    patterns = [
        r'\$\{([^}]+)\}\
...
def process_document_comprehensive(doc, options):
    ...
    try:
        ...
        # Create a list to track table positions in the document
        table_index = 0
        
        for i, table in enumerate(doc.tables):
            try:
                debug_log.append(f"Table {i}: {len(table.rows)} rows x {len(table.rows[0].cells) if table.rows else 0} cols")
                
                if not table.rows:
                    debug_log.append(f"  -> Table {i} is empty, skipping")
                    continue
                
                # Determine number of columns from first row
                num_cols = len(table.rows[0].cells)
                debug_log.append(f"  -> Creating new table with {len(table.rows)} rows x {num_cols} cols")
                
                # Copy table structure
                new_table = new_doc.add_table(rows=len(table.rows), cols=num_cols)
                
                # Copy content và process LaTeX trong từng cell
                for r in range(len(table.rows)):
                    row = table.rows[r]
                    if len(row.cells) != num_cols:
                        debug_log.append(f"    -> Warning: Row {r} has {len(row.cells)} cells, expected {num_cols}")
                    ...
                    for c in range(min(len(row.cells), num_cols)):
                        try:
                            ...
                            debug_log.append(f"    -> Cell [{r}][{c}]: '{cell_text[:30]}{'...' if len(cell_text) > 30 else ''}'")
                            ...
                            if cell_text:
                                ...
                                if latex_count > 0:
                                    debug_log.append(f"      -> Processed {latex_count} LaTeX expressions")
                            else:
                                # Empty cell
                                new_cell.text = ""
                            ...
                            # Set fallback text
                            try:
                                new_table.rows[r].cells[c].text = original_cell.text if 'original_cell' in locals() else ""
                            except:
                                pass
                ...
                if options.get('format_tables', True):
                    try:
                        ...
                        if formatted:
                            tables_formatted += 1
                            # Note: additional_latex might double-count, so we don't add it again
                            debug_log.append(f"  -> Table {i} formatted successfully")
                        else:
                            debug_log.append(f"  -> Table {i} formatting failed")
                    except Exception as format_error:
                        debug_log.append(f"  -> ERROR formatting table {i}: {str(format_error)}")
                
            except Exception as e:
                debug_log.append(f"  -> ERROR in table {i}: {str(e)}")
                # Try to create an empty table as fallback
                try:
                    new_doc.add_table(rows=1, cols=1)
                    new_doc.tables[-1].rows[0].cells[0].text = f"Error processing table {i}"
                except:
                    pass
...
def main():
    ...
    <div style="text-align: center; padding: 20px; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); border-radius: 10px; margin-bottom: 30px;">
        <h1 style="color: white; margin: 0;">📚 Xử lý LaTeX & Format trong Word - v6.2</h1>
        <p style="color: white; margin: 10px 0 0 0;">Chuyển đổi LaTeX, Format câu hỏi trắc nghiệm & Xử lý bảng (Nâng cao với nhiều ký hiệu LaTeX)</p>
    </div>
...
Và câu hỏi:
Câu 7. Nhóm có giá trị đại diện bằng 7 là
A. $[6;8]$.
B. $[2;4]$.  
C. $[4;6]$.
D. $[8;10]$.
        ...
        with st.expander("🔧 Debug LaTeX"):
            st.code("""
Các pattern LaTeX được hỗ trợ:
