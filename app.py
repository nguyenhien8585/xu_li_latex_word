...
def find_math_expressions(text):
    """Tìm expressions toán học trong text - Improved"""
    expressions = []
    i = 0
...
def create_prime_omml(paragraph, text):
    """OMML cho prime notation - Enhanced version"""
    try:
        # Multiple patterns to handle different prime formats
        patterns = [
            r"([A-Za-z]+)('*)",  # B'  or B''
...
def create_equation_object(paragraph, latex_text):
    ...
    # Determine equation type with multiple checks for prime notation
...
def create_word_table(doc, table_data, options):
    """Tạo Word table - Improved version with better formatting"""
    if not table_data:
        return 0, {'omml': 0, 'field': 0, 'styled': 0, 'fallback': 0, 'error': 0}
    ...
    # Fill data with improved cell processing
...
with st.sidebar:
    ...
    if test_latex:
        try:
            ...
            if is_prime:
                ...
                st.info("ℹ️ Will create: Safe OMML Simple Math")
        except Exception:
    ...
    **All Prime Formats Now Work:**
    - `$B^{prime}import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import qn
...
with st.sidebar:
 ...
 → **B'** ✅
    - `$C^{prime}import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import qn
...
with st.sidebar:
 ...
 → **C'** ✅  
    - `$D^{prime}import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import qn
...
with st.sidebar:
 ...
 → **D'** ✅
    ...
    - `$A^{prime}B^{prime}import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import qn
...
with st.sidebar:
 ...
 → **x²** (superscript)  
    - `$\\left(x+1\\right)import streamlit as st
from docx import Document
from docx.shared import Pt, RGBColor
from docx.oxml import parse_xml
from docx.oxml.ns import qn
...
def create_prime_omml(paragraph, text):
    """OMML cho prime notation - Improved version"""
    try:
        # Tìm base và số lượng prime
        prime_match = re.search(r"([A-Za-z]+)('*)", text)
        if not prime_match:
            return create_simple_omml(paragraph, text)
        ...
        # Safe XML with proper escaping
        safe_base = base.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;')
        safe_prime = primes  # Giữ nguyên dấu '
        
        # Tạo OMML với prime notation đúng cách
        omml_xml = f'''<m:oMath xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math">
            <m:sSup>
                <m:sSupPr></m:sSupPr>
                <m:e>
...
def create_equation_object(paragraph, latex_text):
    """Tạo equation object với safe methods - Improved prime detection"""
    unicode_content = convert_latex_symbols(latex_text)
    
    # Determine equation type with better prime detection
    equation_type = 'simple'
    
    # Check for prime notation first (highest priority)
    if re.search(r"[A-Za-z]+'", unicode_content):
        equation_type = 'prime'
    elif '(' in unicode_content and ')' in unicode_content and '/' in unicode_content:
        equation_type = 'fraction'
    elif any(c in unicode_content for c in ['²', '³', '⁴', '⁵', '⁶', '⁷', '⁸', '⁹', '⁰', '¹']):
...
with st.sidebar:
    ...
    st.markdown("### ✅ Prime Notation Examples")
    st.markdown("""
    **Fixed Prime Issues:**
    - `$B^{prime}$` → **B'** ✅
    - `$C^{prime}$` → **C'** ✅  
    ...
    **All equations become TRUE Word objects!**
    """)
...
if uploaded_file:
 ...
 and brace_depth == 0:
    ...
    # Cũng tìm các text có (prime) không nằm trong $ 
