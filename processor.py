...
Phiên bản đơn giản - không cần thư viện phức tạp
"""
...
def analyze_exam_structure(doc):
    ...
    for para in doc.paragraphs:
        ...
        if re.search(r'\|[^|\n]*\|', text):
            ...
            analysis['table_count'] += len(re.findall(r'\|[^|\n]*\|', text))
...
def process_docx(uploaded_file):
    ...
    if analysis['math_count'] > 0:
        stats['processing_details'].append(f"Phát hiện {analysis['math_count']} công thức LaTeX")
    
    if analysis['table_count'] > 0:
        stats['processing_details'].append(f"Phát hiện {analysis['table_count']} bảng Markdown")
